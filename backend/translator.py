...
def detect_language_code(text: str) -> str:
    """
    Detects the language code of the text. Falls back to 'en' on failure.
    """
...
def get_available_ollama_model() -> str:
    """
    Checks if Ollama is running and finds a suitable model to use.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Ollama running. Llama3 not found, using available model: {models[0]}")
            return models[0]
...
def rule_based_ai_fallback(message_english: str) -> Tuple[str, str]:
    ...
    critical_keywords = ["urgent", "not working", "broken", "fail", "error", "down", "crash", "stop", "payment", "charge", "card", "money", "bug", "stuck", "cannot login", "login issue"]
    positive_keywords = ["thanks", "thank you", "great", "awesome", "perfect", "good", "solved", "fixed", "appreciate"]
